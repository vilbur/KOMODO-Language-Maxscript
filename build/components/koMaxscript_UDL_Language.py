# Registers the Maxscript language in Komodo.

import logging
from koUDLLanguageBase import KoUDLLanguage

log = logging.getLogger("koMaxscriptLanguage")
#log.setLevel(logging.DEBUG)

def registerLanguage(registry):
    log.debug("Registering language Maxscript")
    registry.registerLanguage(KoMaxscriptLanguage())

class KoMaxscriptLanguage(KoUDLLanguage):

    # ------------ Komodo Registration Information ------------ #

    name = "Maxscript"
    lexresLangName = "Maxscript"
    _reg_desc_ = "%s Language" % name
    _reg_contractid_ = "@activestate.com/koLanguage?language=%s;1" % name
    _reg_categories_ = [("komodo-language", name)]
    _reg_clsid_ = "13b828a7-e4af-4a72-aa56-a43be71d5312"
    defaultExtension = '.ms'
    ...
    #           'M': 'XML',             #   XML
    #        }
    lang_from_udl_family = {'SSL': 'Maxscript'}
